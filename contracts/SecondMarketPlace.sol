...
contract SecondMarketPlace{

    event OfferingPlaced(bytes32 indexed offeringId, address indexed hostContract, address indexed offerer,  uint tokenId, uint price, uint _amount);
    ...
    mapping (address => bytes32[]) addressOfferings;

    constructor (address _operator) {
        operator = _operator;
    }

    function placeOffering (address _offerer, address _hostContract, uint _tokenId, uint _price, uint _amount) external {
        require(_offerer == msg.sender, "You can not place offers for other people!");
        ...
        offeringNonce += 1;
        addressOfferings[msg.sender].push(offeringId);
        emit OfferingPlaced(offeringId, _hostContract, _offerer, _tokenId, _price, _amount);
    }
    ...
    function closeOffering(bytes32 _offeringId) external payable {
        ...
        ERC1155 hostContract = ERC1155(offeringRegistry[_offeringId].hostContract);
        hostContract.safeTransferFrom(offeringRegistry[_offeringId].offerer, msg.sender, offeringRegistry[_offeringId].tokenId, offeringRegistry[_offeringId].amount, "test");
    ...
    function viewOfferingsForAddress(address _address) external view returns(bytes32[] memory) {
        return addressOfferings[_address];
    }
